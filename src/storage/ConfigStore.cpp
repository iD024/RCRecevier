...
void ConfigStore::load(ReceiverConfig& config) {
    const ReceiverConfig* flashConfig = reinterpret_cast<const ReceiverConfig*>(FLASH_CONFIG_ADDR);
    
    if (flashConfig->magic == CONFIG_MAGIC &&
        flashConfig->checksum == calculateChecksum(*flashConfig)) {
        std::memcpy(&config, flashConfig, sizeof(ReceiverConfig));
    } else {
        loadDefaults(config);
    }
}
...
void ConfigStore::save(const ReceiverConfig& config) {
    ...
    std::memcpy(&toSave, &config, sizeof(ReceiverConfig));
